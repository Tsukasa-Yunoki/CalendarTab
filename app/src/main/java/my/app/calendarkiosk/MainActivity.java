...
// アプリに戻った時に即座に更新を開始
    ...
    private void showLayoutMenu() {
        ...
        String[] items = {"メイン幅(%)", "概要高さ(px)", "メイン左列上限", "サイド左列上限"};

        new AlertDialog.Builder(this).setItems(items, (d, i) -> {

            if(i==0) showSB(items[i], K_W, 20, 95, 67);

            else if(i==1) showSB(items[i], K_H, 40, 500, 100);

            else if(i==2) showSB(items[i], K_FOLD, 1, 20, 6);
